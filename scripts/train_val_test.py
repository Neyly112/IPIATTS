...
TRANSCRIPTION_FILE = os.path.join(
    AUDIO_TEXT_FILE_LIST_PATH, "_all_normal_ipa.txt")
DATA = pd.read_csv(TRANSCRIPTION_FILE, sep=FIELD_SEP,
...
TRAIN_SET_FILE = os.path.join(
    AUDIO_TEXT_FILE_LIST_PATH, "audio_text_train_filelist.txt")
VAL_SET_FILE = os.path.join(
    AUDIO_TEXT_FILE_LIST_PATH, "audio_text_val_filelist.txt")
TEST_SET_FILE = os.path.join(
    AUDIO_TEXT_FILE_LIST_PATH, "audio_text_test_filelist.txt")
...
assert TRAIN_SIZE + VAL_SIZE + \
    TEST_SIZE == 1, "Train, validation, and test sizes must add up to 1."
...
def save_csv(df: pd.DataFrame, filename: str) -> None:
    df.to_csv(filename,            columns=[
              "audio", "text"], sep=FIELD_SEP, index=False, header=False)
    df.to_csv(filename+".cleaned",
              columns=["audio", "text", "ipa"], sep=FIELD_SEP, index=False, header=False)
